...
def main(path):
    """Run the entire DETR pipeline with only an image as argument

    Args:
        path (str or image): path to image or PIL.Image object
    ...
    if type(path) == str:
        ...
        im = path  # passed an image

    scores, boxes = detect(im, detr, transform)
    return scores, boxes


def plot_box(im, scores, boxes, save=None):
    """Plot an image with all objects detected

    Args:
        im (PIL.Image): opened image
        scores (list): list of scores from DETR
    ...
    for p, (xmin, ymin, xmax, ymax), c in zip(scores, boxes.tolist(), COLORS * 100):
        ...
        ax.text(xmin, ymin, text, fontsize=10, bbox=dict(facecolor="yellow", alpha=0.3))
    plt.axis("off")
    if save:
        plt.savefig(save)
    else:
